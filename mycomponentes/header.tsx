...
export default function Header() {
  ...
  {['React', 'React Native', 'Node.js', 'TypeScript', 'Python', 'Scraping', 'Supabase', 'Firebase'].map((stack) => (
    <span key={stack} className="bg-gray-300 text-gray-800 px-3 py-1 rounded-full text-sm transition-colors duration-200 hover:bg-gray-400 hover:text-white">
      {stack}
    </span>
  ))}
</div>
          ...
          <div className="relative w-40 h-40 transition-all duration-1000 delay-300 ease-in-out transform scale-0" style={{ transform: isVisible ? 'scale(1)' : 'scale(0)' }}>
            <Image
              src="/ryan01.jpg?height=160&width=160"
              alt="Profile Picture"
              layout="fill"
